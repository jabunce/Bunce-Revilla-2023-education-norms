...
############ alternate density plotting function
...
denschart4 <- function (x, labels = NULL, groups = NULL, gdata = NULL, cex = par("cex"), 
    ...
    gcolor = par("fg"), lcolor = "gray", xlim = range(unlist(x)), yvals = 1:length(x), yextra=0.7,
    main = NULL, xlab = NULL, ylab = NULL, height=0.7 , border=NA, adjust=1,
    meanlines=FALSE, meanlinesCol=NULL, meanlinesLwd=NULL, ...) 
  {
    opar <- par("mai", "mar", "cex", "yaxs")
    on.exit(par(opar))
    par(cex = cex, yaxs = "i")
    ...
    for ( i in 1:n ) {
        a <- density( x[[i]] , adjust=adjust )
        a$y <- a$y/max(a$y) * height + y[i] - 0.3
        polygon( a$x , a$y , col=color , border=polyborder,lwd=2 ) 
        Cuts <- HPDI( x[[i]] , HPDI)
        XX <- a$x[which(a$x > Cuts[1] & a$x < Cuts[2])]
        YY <- a$y[which(a$x > Cuts[1] & a$x < Cuts[2])]
        polygon( c(min(XX), XX, max(XX)), c(min(a$y), YY, min(a$y)),
          col=colorHPDI, border=polyborderHPDI,lwd=2 )
        if (meanlines == TRUE) {
            lines(x=list( x=c(mean(x[[i]]),mean(x[[i]])), y=c(y[i]-0.28, y[i]-0.3+height+0.2) ),
                    col=meanlinesCol, lty=1, lwd=meanlinesLwd)
        }
    }
...
############ inverted density plotting function


denschart5 <- function (x, labels = NULL, groups = NULL, gdata = NULL, cex = par("cex"), 
    pt.cex = cex, bg = par("bg"), 
    color = "grey20", colorHPDI ="grey60", HPDI=0.9, vline = NULL, polyborder=NA, polyborderHPDI=NA,
    gcolor = par("fg"), lcolor = "gray", xlim = range(unlist(x)), yvals = 1:length(x), yextra=0.7,
    main = NULL, xlab = NULL, ylab = NULL, height=0.7 , border=NA, adjust=1,
    meanlines=FALSE, meanlinesCol=NULL, meanlinesLwd=NULL, ...) 
  {
    ...
    if (is.null(groups)) {
        ...
        y <- yvals #o                           #vertical spacing
        #ylim <- c(0.2, max(y) + yextra) #n + 1)
        #ylim <- -1*c(max(y) + yextra, 0.2)
        ylim <- c(-1*max(y) - yextra, max(y) + yextra)
    }
    else {
        # sub-groups, so need more rows
        o <- sort.list(as.numeric(groups), decreasing = TRUE)
    ...
    for ( i in 1:n ) {
        a <- density( x[[i]] , adjust=adjust )
        a$y <- a$y/max(a$y) * height + y[i] #- 0.3
        polygon( a$x , -1*a$y , col=color[i] , border=polyborder[i],lwd=2 ) 
        Cuts <- HPDI( x[[i]] , HPDI)
        XX <- a$x[which(a$x > Cuts[1] & a$x < Cuts[2])]
        YY <- a$y[which(a$x > Cuts[1] & a$x < Cuts[2])]
        polygon( c(min(XX), XX, max(XX)), -1*c(min(a$y), YY, min(a$y)),
          col=colorHPDI[i], border=polyborderHPDI[i],lwd=2 )
        if (meanlines == TRUE) {
            lines(x=list( x=c(mean(x[[i]]),mean(x[[i]])), y=-1*c(y[i]-0.28, y[i]-0.3+height+0.2) ),
                    col=meanlinesCol, lty=1, lwd=meanlinesLwd)
        }
    }
...
############# complement to inverted density plot


denschart6 <- function (x, labels = NULL, groups = NULL, gdata = NULL, cex = par("cex"), 
    pt.cex = cex, bg = par("bg"), 
    color = "grey20", colorHPDI ="grey60", HPDI=0.9, vline = NULL, polyborder=NA, polyborderHPDI=NA,
    gcolor = par("fg"), lcolor = "gray", xlim = range(unlist(x)), yvals = 1:length(x), yextra=0.7,
    main = NULL, xlab = NULL, ylab = NULL, height=0.7 , border=NA, adjust=1,
